...
Una matriz de núcleo o matriz de Gram o matriz de kernel $K$ puede ser pensada informalmente como una matriz semidefinida positiva de similaridad de a pares entre puntos de un conjunto de datos. Para un conjunto de datos $\{x_1,...,x_m\}$ ésta noción de similaridad esta dada en términos de una función $k$ llamada kernel tal que:
\begin{equation}
	K = K_{ij} = k(x_i, x_j)
\end{equation}
...
\hl{incorporar la imagen de la red de muestra}\\\\
...
\begin{sidewaysfigure}[t!]
    ...
    \end{subfigure}            
    \label{fig:ktalocal}
    \caption{Mapas de colores o heatmaps para caracterización de KTA local para tratamiento 'Frío'. Las zonas rojas presentan mayor densidad de puntos que las azules.\hl{densidad de puntos?binning?algo de esto?}}
\end{sidewaysfigure}
